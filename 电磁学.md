...
比如电场沿导体闭合回路的环量即为所谓感应电动势（回忆一下，我们有 $U=Ed$），根据法拉第定律，它等于负的磁通量变化率。
...
考虑电容 $C$ 和电感 $L$，我们有 $U_C+U_L=0$，且 $U_C=q/C$，$U_L=L\frac{\mathrm di}{\mathrm{d}t}=L\frac{\mathrm d^2q}{\mathrm{d}t^2}$。

于是得到 $\frac{1}{LC}q+\frac{\mathrm d^2q}{\mathrm{d}t^2}=0$。

这是一个经典的波动方程，角频率 $\omega=\frac{1}{\sqrt{LC}}$，即 $q=\sin(\omega t)$。

即周期 $T=2\pi/\omega=2\pi\sqrt{LC}$。整个过程没有用到 Maxwell 方程组。